...
OUT='Successfully copied .shrc folder to home directory ...'
printf $CYN"Copying .shrc folder to home directory ..."$END
    sudo -u "$SUDO_USER" cp -r .shrc "$USER_HOME" || OUT='Failed to copy .shrc folder to home directory ...'
    printf $CYN
    echo $OUT
    printf $END
...
dnf remove -y \
    ...
    gnome-weather \
    ptyxis \
    rhythmbox || OUT='Failed to remove packages from system ...'
    dnf autoremove -y
    printf $CYN
    echo $OUT
    printf $END
...
OUT='Successfully added RPM Fusion to software metadata ...'
printf $CYN"Adding RPM Fusion to appstream metadata repository ..."$END
    dnf group upgrade core || $OUT='Failed to add RPM Fusion to appstream metadata ...'
    printf $CYN
    echo $OUT
    printf $END
